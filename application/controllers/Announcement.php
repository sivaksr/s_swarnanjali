   ...
   public function class_student_list(){
	...
	public  function smspost(){
		if($this->session->userdata('userdetails'))
		{
		    $post=$this->input->post();
			//echo '<pre>';print_r($post);exit;
			if($post['student_name']!='ALL'){
			foreach($post['student_name'] as $li){ 
			$student_list_data=$this->Announcement_model->get_student_list_data(ucfirst($li));
						echo '<pre>';print_r($student_list_data);exit;
            
			$names[]=$student_list_data['mobile'];
			}
			}else{
			$data['student_list']=$this->Announcement_model->get_student_list($post['class_id']);
			echo '<pre>';print_r($data);exit;
			...
			if($post['student_name']=='ALL'){
				...
				$student_list=$this->Announcement_model->get_student_list($post['class_id']);
				echo '<pre>';print_r($student_list);exit;
				foreach($student_list as $list){
				$mobiles[]=$list['mobile'];
			}
			$los=implode(',', $mobiles);
				...
				//echo '<pre>';print_r($post);exit;
			foreach($post['student_name'] as $li){ 
						$data['student_list_data']=$this->Announcement_model->get_student_list_data(ucfirst($li));
						
					
					}
//echo '<pre>';print_r($data);exit;
			//$data['student_list_data']=$this->Announcement_model->get_student_list_data($lop);
